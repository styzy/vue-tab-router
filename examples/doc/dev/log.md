...
* 更新内容：修复标题组件在关闭非聚焦的路由视图后，会失去聚焦路由视图的bug。

## 版本： `0.8.3`

* 更新时间： `2023-12-08`

* 更新内容：修复标题组件`<TabRouterLink>`无法正确提供作用域插槽参数`isVisited`的bug。

## 版本： `0.8.2`

* 更新时间： `2023-12-08`

* 更新内容：修复关闭所有路由后，数据无法同步更新相应的bug。

## 版本： `0.8.1`

* 更新时间： `2023-12-08`

* 更新内容：优化导航守卫，现在通过`focus()`方法聚焦已经处于聚焦状态的路由时，可以成功触发`全局前置导航守卫（beforeEach）`的`聚焦类型（NAVIGATE_TYPES.FOCUS）`了，但因为此聚焦行为属于无意义的聚焦行为，为了避免不必要的的性能消耗，因此`全局后置钩子`将`不会`被触发。
